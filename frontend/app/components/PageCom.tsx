...
import StepProvider, { StepContext } from "@/app/context/step";

const Chat = dynamic(() => import("../components/Ably"), {
  ssr: false,
});

export default function PageCom({ params }) {
  let { roomId } = params;
  ...
  const { step, setStep, changeStep, broadcast, changeBroadcast } = stepCon;
  ...
  const startGame = () => {
    ...
    // setNewMessage('');
    console.log("🚀 ~ startGame ~", "question");
    setStep("question");
  };
  ...
  useEffect(() => {
    const answerQ = async (input) => {
      ...
      const allP = await createPlayerScore({
        score: input.score,
      ...
      setStep("score");
    //   let isAllPlayerAnswered = true;
    //   for (const p of allPlayers) {
    //     if (!allP.map((v) => v.player_id).includes(p.id)) {
    //       isAllPlayerAnswered = false;
    //     }
    //   }
    //   if (isAllPlayerAnswered) {
    //     console.log(
    //       "🚀 ~ answerQ ~ waitForScore:",
    //       "all player answered ,broadcast"
    //     );
    //     changeBroadcast('score')
    //   } else {
    //   }
    };
      ...
      <div className="bg-[url('/bg-2.png')] h-[100vh] bg-cover">
        ...
        {step === "question" && (
          <div className="flex flex-col items-center">
            <div className="bg-white w-full font-bold p-5">
              Questions:{" "}
              {gameQs[questionNum]?.QuestionBank?.question_text || ""}
            </div>
            <div className="mt-5">
              <Countdown
        ...
        {step === "score" && (
          <div className={cn("flex flex-col items-center")}>
            <div
              className={cn(
                "bg-white w-full p-5 font-bold flex justify-center",
                {
            ...
            <div className="bg-[#002265] min-w-[300px] min-h-[400px] p-2 mt-12 text-white flex flex-col gap-2 rounded font-bold">
              {scoreList.sort((a,b)=>b.score - a.score).map((s) => (
